...
AnimoSort reads that EAF locally, normalizes the meeting rows, sorts them by their actual start times, and renders a Monday to Saturday timetable. The [About page](about.html) shows the supplied Archershub output, the source receipt, and the generated result side by side.
